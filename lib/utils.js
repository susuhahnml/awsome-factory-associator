...
module.exports = {
  ...
  generateFromOptionsObjectAfter: (method, factory, optionsObject, associationConfig, createdInstance, saved) => {
    //Allow options using saved
    optionsObject = getIfSavedAtttribute(saved, optionsObject);
    ...
    if (assingedById) {
      throw new Error('Ids can not be passed in '+associationConfig.as+' since it is a HasOne or HasMany association.');
    }
